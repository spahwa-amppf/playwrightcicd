...
class LoginPage {
  ...
  async navigate(route = '') {
    ...
    async navigateDefault() {
        ...
        await this.page.goto( envUrls['dev']);
    }
...
// }
  ...
  async login(email, password) {
    ...
    await this.page.waitForSelector('#Email', { timeout: 10000 }); // Wait before filling
  ...
  async login_uidev2 (email, password, route) {
      ...
      await this.page.waitForSelector('#Email', { timeout: 10000 }); // Safe wait
      await this.emailInput.fill(email );
      await this.page.getByText('Next').click();
      await this.passwordInput.fill(password);
      await this.loginButton.click();
